...
const editData = ref({ ...props.product });
  ...
  <div v-if="isOpen" class="modal-overlay" @click.self="$emit('close')">
    <div class="modal-glass">
      ...
      <form @submit.prevent="handleUpdate" class="edit-form">
        ...
        <div class="field">
          <label>Product Name LC</label>
          <input v-model="editData.name_lc" type="text" />
        </div>
        <div class="field">
            <label>Description</label>
            <input v-model="editData.descriptions" type="text" />
          </div>
        
        <div class="row">
          <div class="field">
            <label>Price</label>
            <input v-model="editData.price" type="number" step="0.01" />
          </div>
          <div class="field">
            <label>Stock</label>
            <input v-model="editData.stock_quantity" type="number" />
          </div>
        </div>
...
.modal-overlay {
  position: fixed;
  inset: 0;
  background: rgba(0, 0, 0, 0.85); /* Darken the city background when editing */
  display: flex;
  justify-content: center;
  align-items: center;
  z-index: 9999;
  backdrop-filter: blur(5px); /* Extra blur for the background */
}
...
.modal-glass {
  ...
  box-shadow: 
    ...
    inset 0 0 20px rgba(255, 255, 255, 0.05); /* Inner soft glow */
}
...
.field input:focus {
  border-color: rgba(223, 37, 49, 0.5);
...
/* Manchester United Red Button */
.btn-primary { 
  background: radial-gradient(circle at 30% 30%, #df2531, #b31d27);
  color: white; 
  border: none; 
  padding: 14px; 
  border-radius: 12px; 
    ...
    <div class="modal-glass">
      <header class="modal-header">
        <div class="header-title">
          <div class="icon-box"><ImageIcon :size="18" /></div>
          <h2>Edit Jersey</h2>
        </div>
        <button @click="$emit('close')" class="close-x"><X :size="20" /></button>
      </header>
        ...
        <div class="field">
          ...
          >
            ...
            <div v-if="!previewUrl" class="upload-placeholder">
              <div class="upload-circle"><Upload :size="24" /></div>
...
.modal-glass {
  ...
  width: 90%; max-width: 480px; color: white;
...
.icon-box { background: rgba(223, 37, 49, 0.2); padding: 8px; border-radius: 10px; color: #df2531; }
.modal-header h2 { font-size: 20px; margin: 0; }
.close-x { background: transparent; border: none; color: #aaa; cursor: pointer; transition: 0.2s; }
...
.upload-zone:hover, .drag-active { border-color: #df2531; background: rgba(223, 37, 49, 0.05); }

.upload-placeholder { text-align: center; color: rgba(255, 255, 255, 0.5); }
.upload-circle { color: #df2531; margin-bottom: 8px; }
.upload-placeholder p { font-size: 14px; font-weight: 600; margin: 0; color: white; }
.upload-placeholder span { font-size: 11px; }

.upload-preview { width: 100%; height: 160px; padding: 10px; position: relative; }
.upload-preview img { width: 100%; height: 100%; object-fit: contain; border-radius: 12px; }
.remove-btn {
...
.field label { font-size: 12px; font-weight: 700; color: rgba(255, 255, 255, 0.4); text-transform: uppercase; }
.field input, .field select {
  background: rgba(0, 0, 0, 0.4); border: 1px solid rgba(255, 255, 255, 0.1);
  padding: 12px; border-radius: 12px; color: white; outline: none; transition: 0.2s;
}
.field input:focus { border-color: #df2531; box-shadow: 0 0 10px rgba(223, 37, 49, 0.2); }
.row { display: grid; grid-template-columns: 1fr 1fr; gap: 15px; }

/* Buttons */
.modal-actions { display: flex; gap: 12px; margin-top: 10px; }
.btn-primary { 
  background: #df2531; color: white; border: none; 
  padding: 14px; border-radius: 14px; flex: 1; 
  font-weight: 700; cursor: pointer; transition: 0.3s;
  box-shadow: 0 8px 20px rgba(223, 37, 49, 0.3);
}
.btn-primary:hover { transform: translateY(-2px); box-shadow: 0 12px 25px rgba(223, 37, 49, 0.4); }
.btn-secondary { background: rgba(255, 255, 255, 0.05); color: white; border: none; padding: 14px; border-radius: 14px; flex: 1; cursor: pointer; }
</style>